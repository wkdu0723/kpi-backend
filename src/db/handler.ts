...
export const jiraDataMigration = async (webhookData: JiraWebhookData) => {
    try {
        ...
        if (
            eventType === JiraWebhookEvent.issuelink_created &&
            webhookData.issueLink
        )
            await setJiraIssueLink(webhookData.issueLink);
        else if (
            eventType === JiraWebhookEvent.issuelink_deleted &&
            webhookData.issueLink
        )
            await deleteJiraIssueLink(webhookData.issueLink);
        else if (
            eventType === JiraWebhookEvent.issue_deleted &&
            webhookData.issue
        )
            await deleteIssue(webhookData.issue.id);
        else if (
            eventType === JiraWebhookEvent.worklog_created ||
            eventType === JiraWebhookEvent.worklog_updated ||
            eventType === JiraWebhookEvent.worklog_deleted
        )
            await jiraWorkLogHandler(eventType, webhookData.worklog);
        else if (webhookData.issue)
            await setJiraIntegratedIssue([webhookData.issue]);
    } catch (err) {
...
export const projectDataMigration = (
    issueData: JiraIssueData
): ProjectDataMigrationResult => {
    ...
    const {
        ...
        colorName: status_category_color,
    } = statusCategory;
    const assignee_account_id = assignee?.accountId
        ? assignee.accountId
        : creator.accountId;
    const assignee_display_name = assignee?.displayName
...
export const setAccountProjectHandler = async (
    accountId: string,
    email: string,
    accountAPIKey: string
) => {
    try {
        const resp = await requestAccountProject(
            accountId,
            email,
            accountAPIKey
        );
...
export const setJiraAccountHandler = async (
    accountId: string,
    name: string,
    email: string,
    accountAPIKey: string
...
export const jiraWorkLogHandler = async (
    eventType: JiraWebhookEvent,
    workLog?: JiraWorkLogData
) => {
    try {
        if (!workLog) return;

        if (eventType === JiraWebhookEvent.worklog_deleted)
            await deleteJiraWorkLog(workLog.id);
